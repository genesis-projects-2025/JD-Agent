...
async def get_all_feedback(db: AsyncSession = Depends(get_db)):
    try:
        ...
        query = (
            ...
            .outerjoin(Employee, Feedback.employee_id == Employee.id)
            .order_by(desc(Feedback.created_at))
        )
            ...
            formatted_list.append(
                {
                    ...
                    "jd_session_id": str(feedback.jd_session_id)
                    if feedback.jd_session_id
                    else None,
...
async def update_feedback_status(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Feedback status updated to {request.status}",
        }
