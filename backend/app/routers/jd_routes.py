...
async def hydrate_session_from_db(session_id: str, db: AsyncSession) -> SessionMemory:
    ...
    if record:
        ...
        memory.employee_name = record.employee_name  # ← restore
...
async def init_jd(request: InitJDRequest, db: AsyncSession = Depends(get_db)):
    ...
    memory.employee_name = request.employee_name  # ← store name
    ...
    await sync_session_to_db(
        ...
        employee_name=request.employee_name,  # ← pass to DB
    )
...
async def chat(request: ChatRequest, db: AsyncSession = Depends(get_db)):
    ...
    await sync_session_to_db(
        ...
        employee_name=session_memory.employee_name,  # ← added
