...
async def get_hierarchy(request: HierarchyRequest = Body(default=HierarchyRequest()), db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        def get_all_descendants(start_territory: str) -> set:
            ...
            while queue:
                ...
                children = [
                    r for r in rows 
                    if r.get("Area_Name") and r.get("Area_Name").strip().lower() == current.strip().lower()
                ]
        ...
        def build_node(terr: str):
            ...
            child_rows = [
                ...
                if r.get("Area_Name") and r.get("Area_Name").strip().lower() == terr.strip().lower()
            ]
        ...
        if territory:
            ...
            top_levels = [r for r in filtered_rows if r.get("Territory", "").strip() not in child_areas]
