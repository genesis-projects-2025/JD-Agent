...
import {
  ...
  Calendar, Tag, Loader2, FileText, RotateCcw,
  CheckCheck, AlertCircle, ArrowLeft,
} from "lucide-react";
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-neutral-200 w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-neutral-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-neutral-900">Return for Revision</h3>
              <p className="text-sm text-neutral-500">{jd.role_title} — {jd.employee_name}</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-6 py-5 border-b border-neutral-100 space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-neutral-900">{jd.role_title}</h2>
            ...
            <span className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full flex-shrink-0 ${sc.bg} ${sc.text}`}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-neutral-50 rounded-xl p-3">
              ...
              <div className="text-sm font-medium text-neutral-700">{formatDate(jd.created_at)}</div>
            </div>
            <div className="bg-neutral-50 rounded-xl p-3">
              <div className="text-xs text-neutral-400 mb-0.5">Last updated</div>
              <div className="text-sm font-medium text-neutral-700">{timeAgo(jd.updated_at)}</div>
            </div>
          </div>

          {skills.length > 0 && (
            <div>
              <div className="text-xs text-neutral-400 mb-2">Required Skills</div>
          ...
          <div>
            <div className="flex items-center justify-between text-xs mb-1.5">
              ...
              <span className="font-medium text-neutral-600">{jd.completion_percentage}%</span>
            </div>
        ...
        {jd.status === "rejected" && jd.reviewer_comment && (
          <div className="mx-6 my-4 p-4 bg-rose-50 border border-rose-200 rounded-xl">
            ...
            <p className="text-sm text-rose-700 leading-relaxed">{jd.reviewer_comment}</p>
            {jd.reviewed_at && (
              <p className="text-xs text-rose-400 mt-2">{timeAgo(jd.reviewed_at)} by {jd.reviewed_by}</p>
            )}
          </div>
        )}
        ...
        {jd.jd_text ? (
          <div className="px-6 pb-6">
            <div className="text-xs text-neutral-400 mb-3 mt-2">Job Description Content</div>
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-sm text-neutral-900 truncate">{jd.role_title}</div>
          <div className="text-xs text-neutral-500 mt-0.5">{jd.employee_name} · {jd.department}</div>
        </div>
        <span className={`flex items-center gap-1 text-[11px] font-semibold px-2 py-1 rounded-full flex-shrink-0 ${sc.bg} ${sc.text}`}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 w-20">
          <div className="flex-1 h-1 bg-neutral-100 rounded-full overflow-hidden">
            <div className="h-full bg-blue-400 rounded-full" style={{ width: `${jd.completion_percentage}%` }} />
          </div>
          <span className="text-[10px] text-neutral-400">{jd.completion_percentage}%</span>
        </div>
      </div>
    </button>
  );
...
export default function ApprovalsPage() {
  ...
  const handleApprove = useCallback(async () => {
    ...
    setJds((prev) =>
      prev.map((j) =>
        j.id === selectedId
          ? { ...j, status: "approved", reviewed_by: "You", reviewed_at: new Date().toISOString() }
          : j,
      ),
    );
    setIsActing(false);
  ...
  const handleRejectConfirm = useCallback(async (comment: string) => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 mb-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-neutral-900">JD Approvals</h1>
            ...
            {STATUS_TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === tab.key
                    ? "bg-blue-50 text-blue-700"
                    : "text-neutral-600 hover:bg-neutral-50"
                    }`}
                >
                  <span>{tab.label}</span>
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? "bg-blue-100 text-blue-700" : "bg-neutral-100 text-neutral-500"
                    }`}>
                    {count}
                  </span>
                </button>
        ...
        {selectedJD ? (
          ...
          <div className="flex-1 flex items-center justify-center bg-white rounded-2xl border border-neutral-200 shadow-sm">
            <div className="text-center">
              ...
              <p className="text-neutral-500 font-medium">Select a JD to review</p>
              <p className="text-sm text-neutral-400 mt-1">Choose from the list on the left</p>
            </div>
          </div>
        )}
      </div>
