...
export default function DashboardPage() {
  ...
  }, [teamStats, myTeamEmployees, loadingTeam]);
  ...
  const handleDepartmentClick = async (
    deptName: string,
    submittedOnly: boolean = onlySubmitted,
  ) => {
    setSelectedDepartment(deptName);
    setLoadingDept(true);
    try {
      const data = await fetchDepartmentEmployees(
        deptName,
        1,
        100,
        submittedOnly,
    ...
    <div className="h-[calc(100vh-3rem)] md:h-[calc(100vh)] overflow-y-auto bg-surface-50">
      <div className="max-w-6xl mx-auto pt-20 pb-10 px-4 sm:px-8 md:pt-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl sm:text-4xl font-medium text-surface-900 flex flex-wrap items-center gap-3">
              JD Intelligence
              <span className="text-xs font-medium px-3 py-1 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-md shadow-sm">
                {role}
              </span>
            </h1>
        ...
        <div className="mb-8">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-surface-200">
            <h3 className="text-lg font-medium text-surface-900 mb-4">
              Getting Started with JD Intelligence
            </h3>
            <video
              controls
            ...
            <p className="mt-3 text-sm text-surface-500 text-center">
              Watch this quick introduction to understand how JD Intelligence
              helps you create effective job descriptions
            </p>
          </div>
        </div>
        ...
        <Link
          href="/questionnaire"
          className="group relative overflow-hidden flex items-center justify-between p-8 bg-gradient-to-r from-primary-600 via-primary-700 to-indigo-800 rounded-md text-white shadow-md shadow-primary-900/20 hover:shadow-md hover:shadow-primary-900/40 hover:-translate-y-1 transition-all duration-300 mb-10"
        >
          <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 mix-blend-overlay"></div>
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
            <div className="w-16 h-16 shrink-0 bg-white/10 backdrop-blur-md rounded-md flex items-center justify-center border border-white/20 group-hover:scale-110 transition-transform duration-500">
          ...
          <div className="hidden sm:flex relative z-10 w-12 h-12 shrink-0 rounded-md bg-white/10 items-center justify-center backdrop-blur-md border border-white/20 group-hover:bg-white/20 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {/* Drafts */}
          <div className="bg-white rounded-md p-6 shadow-sm border border-surface-200 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-12 h-12 rounded-md bg-amber-50 text-amber-600 flex items-center justify-center border border-amber-100/50">
              ...
              <div>
                ...
                <p className="text-4xl font-medium text-surface-900 mt-0.5">
                  {draftCount}
                </p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-md p-6 shadow-sm border border-surface-200 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-12 h-12 rounded-md bg-blue-50 text-blue-600 flex items-center justify-center border border-blue-100/50">
          ...
          <div className="bg-white rounded-md p-6 shadow-sm border border-surface-200 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-12 h-12 rounded-md bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-100/50">
        ...
        {(isManager || isHead || isHR) && (
          <div className="flex overflow-x-auto gap-3 mb-8 bg-surface-200/50 p-1.5 rounded-md w-full sm:w-fit custom-scrollbar pb-2 sm:pb-1.5">
            <button
            ...
            {isHR && (
              <>
                ...
                >
                  HR Review Queue
                  {pendingJDs.length > 0 && (
                    <span className="bg-primary-500 text-white text-[11px] px-2 py-0.5 rounded-md font-medium">
                      {pendingJDs.length}
                    </span>
                  )}
                </button>
                ...
                <div className="p-6 sm:p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {departmentStats.map((stat) => (
                    <div
                      ...
                      className="border border-surface-200 rounded-md p-5 bg-surface-50/30 hover:shadow-md hover:border-primary-300 transition-all cursor-pointer group"
                    >
                      <div className="flex justify-between items-start mb-4">
                        <h3 className="text-lg font-medium text-surface-900 group-hover:text-primary-700 transition-colors">
                          {stat.department}
                        </h3>
                      ...
                      <div className="w-full h-3 bg-surface-200 rounded-md overflow-hidden mb-6">
                        <div
                          className="h-full bg-primary-500 rounded-md transition-all duration-1000 ease-out"
        ...
        {activeTab === "departments" && (role === "hr" || role === "admin") && selectedDepartment && (
          <div className="bg-white rounded-md border border-surface-200 shadow-md overflow-hidden mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="px-4 sm:px-8 py-5 sm:py-6 border-b border-surface-100 flex flex-col sm:flex-row sm:items-center justify-between bg-surface-50/50 gap-4">
              ...
              <div className="flex items-center gap-2 bg-surface-200/50 p-1 rounded-md">
                <button
            ...
            {loadingDept ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-surface-50/80 border-b border-surface-200 text-xs font-medium text-surface-500 tracking-wider">
                      <th className="px-6 py-4 font-medium">Employee</th>
                      <th className="px-6 py-4 font-medium">Designation</th>
                      <th className="px-6 py-4 font-medium">
                        Reporting Manager
                      </th>
                      <th className="px-6 py-4 font-medium">JD Status</th>
                    </tr>
                  </thead>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-5">
                        <div className="w-12 h-12 bg-surface-100 rounded-md flex items-center justify-center group-hover:bg-white group-hover:shadow-sm transition-all border border-transparent group-hover:border-surface-200 shrink-0">
                          <Briefcase className="w-6 h-6 text-surface-400 group-hover:text-primary-600 transition-colors" />
                        </div>
                        <div>
                          <h3 className="text-base font-medium text-surface-900 group-hover:text-primary-700 transition-colors">
                            {jdItem.title || "Untitled JD"}
                          </h3>
                          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1.5">
                            <span
                              className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md text-[11px] font-medium border ${config.bg} ${config.color}`}
                            >
                              <span className="w-1.5 h-1.5 rounded-md bg-current" />
                              {config.label}
                            </span>
                            <span className="flex items-center gap-1.5 text-[12px] font-medium text-surface-400">
                            ...
                            <span className="text-[12px] font-medium text-surface-400 bg-surface-100 px-2 py-0.5 rounded-md">
                              v{jdItem.version}
                            </span>
                          </div>
                        </div>
                        ...
                        {activeTab === "my_jds" && (
                          <>
                            <button
                              ...
                              className="p-2 sm:p-2.5 text-surface-400 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors border border-transparent hover:border-red-100"
                              title="Delete JD"
                            >
