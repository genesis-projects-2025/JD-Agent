        ...
        {feedback.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {feedback.map((item, idx) => (
              <div
                key={item.id}
                className={`bg-white rounded-[24px] p-6 border transition-all duration-300 group hover:shadow-premium hover:-translate-y-1 ${
                  !item.is_read
              ...
              >
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="flex flex-col gap-2">
                    ...
                    {!item.is_read && (
                      <span className="w-2 h-2 bg-orange-500 rounded-full animate-pulse absolute -top-1 -right-1 ring-4 ring-white" />
                    )}
                  </div>
