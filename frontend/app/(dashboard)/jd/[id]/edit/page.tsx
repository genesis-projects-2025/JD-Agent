...
export default function JDEditPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Send both text and structured separately as generated by LLM/edited
      await updateJD(jdId, {
        jd_text: jdText,
      ...
      <div className="flex-1 min-h-0 overflow-y-auto bg-neutral-50">
        <div className="max-w-5xl mx-auto px-8 py-8 space-y-6">
          ...
          {structured.team_structure &&
            typeof structured.team_structure === "object" &&
            Object.keys(structured.team_structure).length > 0 && (
              <div className="bg-white rounded-xl border border-neutral-200 shadow-sm">
                ...
                <div className="px-6 py-5 space-y-4">
                  {Object.entries(structured.team_structure).map(([k, v]) => (
                    <div key={k}>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          updateSection("team_structure", newStruct);
                        }}
                        className="w-full px-4 py-2.5 bg-neutral-50 border border-neutral-200 rounded-lg text-sm text-neutral-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      />
                    </div>
                  ))}
                </div>
          ...
          {structured.work_environment &&
            typeof structured.work_environment === "object" &&
            Object.keys(structured.work_environment).length > 0 && (
              <div className="bg-white rounded-xl border border-neutral-200 shadow-sm">
                ...
                <div className="px-6 py-5 space-y-4">
                  {Object.entries(structured.work_environment).map(([k, v]) => (
                    <div key={k}>
                      <label className="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2 block">
                        {k.replace(/_/g, " ")}
                      </label>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          updateSection("work_environment", newStruct);
                        }}
                        className="w-full px-4 py-2.5 bg-neutral-50 border border-neutral-200 rounded-lg text-sm text-neutral-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      />
                    </div>
          ...
          {/* Professional Context (JD Text) */}
          <div className="bg-white rounded-xl border border-neutral-200 shadow-sm">
            <div className="px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-neutral-900 uppercase tracking-wide">
                Professional JD Text
              </h3>
            </div>
            <div className="px-6 py-5">
              <textarea
                ...
                rows={12}
                className="w-full p-4 bg-neutral-50 border border-neutral-200 rounded-xl text-sm text-neutral-800 leading-relaxed resize-y focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                placeholder="Enter professional JD text output..."
              />
            </div>
          </div>
