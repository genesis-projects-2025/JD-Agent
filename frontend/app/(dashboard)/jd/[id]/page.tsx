...
function JDPageContent() {
  ...
  const [activeTab, setActiveTab] = useState<
    "structured" | "kra-kpi" | "skill-assessment"
  >(
    tabParam === "kra-kpi"
      ? "kra-kpi"
      : tabParam === "skill-assessment"
        ? "skill-assessment"
        : "structured",
  );
  ...
  useEffect(() => {
    if (tabParam === "kra-kpi") {
      ...
      const isJdComplete =
        ...
        (jd.status === "approved" ||
          !!jd.generated_jd ||
          !!jd.jd_text ||
          (jd.jd_structured && Object.keys(jd.jd_structured).length > 0));
      if (jd && !isJdComplete) {
      ...
      if (prereqStatus && !prereqStatus.ready) {
    ...
    } else if (tabParam === "skill-assessment") {
      // Check if KRA/KPI is approved before allowing access
      if (jd && jd.kra_kpi_status === "approved") {
        setActiveTab("skill-assessment");
      } else {
        // Not ready, trigger the same prereq popup
        setPrereqMissing(["kra_kpi_status"]);
        setIsPrereqModalOpen(true);
        setActiveTab("structured");
        router.replace(`/jd/${jdId}`);
      }
  ...
  // Resolve identity from the DB, not the client cache. getCurrentUser() reads
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        const u = await syncIdentityFromDB();
        ...
        if (
  data.kra_kpi_status === "sent_to_manager" || 
  data.kra_kpi_status === "sent_to_hr" || 
  tabParam === "kra-kpi"
) {
  setActiveTab("kra-kpi");
} else {
  setActiveTab("structured"); // Default to JD
}
        ...
        if (pStruct && typeof pStruct === "object") {
          ...
          if (pStruct.talent_bar && typeof pStruct.talent_bar === "object") {
            pStruct.education =
              pStruct.education || pStruct.talent_bar.education || "";
            pStruct.experience =
              pStruct.experience || pStruct.talent_bar.experience || "";
          }
          // qualifications nested -> top-level
          if (
            pStruct.qualifications &&
            typeof pStruct.qualifications === "object"
          ) {
            pStruct.education =
              pStruct.education || pStruct.qualifications.education || "";
            pStruct.experience =
              pStruct.experience || pStruct.qualifications.experience || "";
          }
        }
  ...
  if (!isMounted) {
    // Hydration fix: Only render the main UI once mounted
    return (
...
const handleRejectWithModal = async (
  ...
  try {
    // If KRA/KPI is pending, reject the KRA/KPI instead of the JD
    if (jd?.kra_kpi_status === "sent_to_manager" || jd?.kra_kpi_status === "sent_to_hr") {
      ...
      // Otherwise, reject the JD normally
  ...
  // Single source of truth for "who can act on this JD", used everywhere below
  // instead of three separate, slightly-different re-implementations. Falls back
  // to currentUser.role in case the `role` state hasn't caught up yet (e.g. right
  // after syncIdentityFromDB resolves but before the next render settles).
  const effectiveRole = (role || currentUser?.role || "").toLowerCase();
  const isManagerRole =
    ["manager", "head"].includes(effectiveRole) || isManager(currentUser);
  const isHRUser = ["hr", "admin"].includes(effectiveRole) || isHR(currentUser);
  const userEmpIdNorm = (currentUser?.employee_id || "").trim().toUpperCase();
  const mgrCodeNorm = (jd?.reporting_manager_code || "").trim().toUpperCase();
  const isDirectManager =
    !!userEmpIdNorm && !!mgrCodeNorm && userEmpIdNorm === mgrCodeNorm;
  const isInspector =
    !!currentUser?.employee_id && currentUser?.employee_id !== jd?.employee_id;
  const canActOnThisJD =
    isHRUser || (isManagerRole && (isDirectManager || isInspector));
          ...
          onClick={() => {
            ...
            if (jd) {
              ...
              if (
                ...
                (jd.kra_kpi_status && jd.kra_kpi_status.includes("rejected"))
              ) {
                if (!isInspector) {
                  url += "?view=feedback";
                }
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-6">
                ...
                <div className="flex flex-wrap items-center gap-2 border border-surface-200/50 bg-surface-50/50 p-1 rounded-md text-[11px] font-medium shadow-sm">
                  <span className="px-2 text-surface-400 font-bold tracking-wider text-[9px] uppercase">
                    JD Status:
                  </span>
                  <span
                    className={`px-2.5 py-1 rounded-md text-[11px] font-semibold border ${
                      jd.status === "approved"
                        ...
                        : ["sent_to_manager", "sent_to_hr"].includes(jd.status)
                  ...
                  >
                    ...
                    )[jd.status] || jd.status.replace(/_/g, " ")}
                  </span>
                </div>
                ...
                {jd.kra_kpi_status && (
                  <div className="flex flex-wrap items-center gap-2 border border-surface-200/50 bg-surface-50/50 p-1 rounded-md text-[11px] font-medium shadow-sm">
                    ...
                    <span
                      className={`px-2.5 py-1 rounded-md text-[11px] font-semibold border ${
                        jd.kra_kpi_status === "approved"
                          ? "bg-emerald-50 text-emerald-700 border-emerald-100"
                          : ["sent_to_manager", "sent_to_hr"].includes(
                                jd.kra_kpi_status,
                              )
                            ? "bg-amber-50 text-amber-700 border-amber-100 animate-pulse"
                            : jd.kra_kpi_status.includes("rejected")
                              ? "bg-rose-50 text-rose-700 border-rose-100"
                              : "bg-primary-50 text-primary-700 border-primary-100"
                      }`}
                    >
              ...
              {jd.jd_structured && (
                <div className="relative mt-4 inline-block">
                  ...
                  {showDownloadDropdown && (
                    <div className="absolute left-0 mt-2 w-56 bg-white border border-surface-200 rounded-md shadow-md z-[100] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
  ...
  <>
    {(() => {
      ...
      return (
        ...
        {(() => {
          // isManagerRole / isHRUser / isDirectManager are the shared, DB-backed
          // checks computed once near the top of this component.
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700 leading-relaxed">
                      ...
                      has submitted their Job Description for your review.
                      Please examine the role purpose, responsibilities, and
                      qualifications, then Approve, Reject, or click Edit above
                      to modify details directly.
                    </p>
                  </div>
                </div>
              </div>
          ...
          if (isHRUser && jd.status === "sent_to_hr") {
            return (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-purple-900 mb-1">
                      Job Description Pending HR Sign-Off
                    </h3>
                    <p className="text-sm text-purple-700 leading-relaxed">
                      ...
                      is awaiting your final sign-off. Please review the
                      details. You can make adjustments by clicking Edit above,
                      or Approve/Reject this Job Description.
                    </p>
                  </div>
                </div>
              </div>
            ...
            return (
              <div className="bg-amber-50 border-2 border-amber-200 rounded-md md:rounded-[32px] p-6 mb-6 shadow-sm animate-in fade-in duration-300">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-amber-900 mb-1">
                      KRA & KPI Framework Pending Approval
                    </h3>
                    <p className="text-sm text-amber-700 leading-relaxed">
                      The Job Description is finalized. Employee{" "}
                      <strong className="text-amber-900 font-semibold">
                        {jd.employee_name}
                      </strong>{" "}
                      has now submitted their performance KRA/KPI targets and
                      expectations for your approval. Switch to the{" "}
                      <strong>KRA / KPI</strong> tab below to review. You can
                      click <strong>Edit</strong> at the top to tweak targets,
                      weights, and metrics directly.
                    </p>
                  </div>
                </div>
              </div>
            ...
            return (
              <div className="bg-purple-50 border-2 border-purple-200 rounded-md md:rounded-[32px] p-6 mb-6 shadow-sm animate-in fade-in duration-300">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-purple-900 mb-1">
                      KRA & KPI Framework Pending HR Review
                    </h3>
                    <p className="text-sm text-purple-700 leading-relaxed">
                      The performance framework (KRAs & KPIs) for{" "}
                      <strong className="text-purple-900 font-semibold">
                        {jd.employee_name}
                      </strong>{" "}
                      is awaiting your final review and sign-off. Switch to the{" "}
                      <strong>KRA / KPI</strong> tab below to inspect targets
                      and weights. You can edit directly or sign off on the
                      framework.
                    </p>
                  </div>
                </div>
        ...
        {(() => {
          ...
          const latestRejection = [...reviewComments]
            ...
            .find(
              (c: any) =>
                c.action === "rejected" && rolesMatch(c.target_role, role),
            );
          ...
          const reviewerLabel = rejectedByHR
            ...
            : rejectedByManager
              ? "your Manager"
              : latestRejection.reviewer_role === "hr" ||
                  latestRejection.reviewer_role === "admin"
                ? "HR"
                : "your Manager";
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-red-700 text-sm leading-relaxed mb-4">
                    <strong>
                      {latestRejection.reviewer_name || reviewerLabel}
                    </strong>{" "}
                    ({reviewerLabel}) requested changes to this document:
                  </p>
        ...
        <div className="bg-white rounded-md md:rounded-[32px] border border-surface-200 shadow-md overflow-hidden transition-all duration-500">
          ...
          {!isEditing && (
            <div className="flex border-b border-surface-100">
                ...
                onClick={async () => {
                  const isJdComplete =
                    ...
                    (jd.status === "approved" ||
                      ...
                      !!jd.jd_text ||
                      (jd.jd_structured &&
                        Object.keys(jd.jd_structured).length > 0));
                  if (!isJdComplete) {
                    setPrereqMissing(["employee_jd"]);
                    setIsPrereqModalOpen(true);
                    return;
                  ...
                  if (!currentStatus && jd) {
                    try {
                      currentStatus = await fetchKRAKPIStatus(
                        jdId,
                        jd.employee_id,
                      );
              ...
              <button
                onClick={() => {
                  // Check status before allowing click
                  if (jd && jd.kra_kpi_status === "approved") {
                    setActiveTab("skill-assessment");
                  } else {
                    setPrereqMissing(["kra_kpi_status"]);
              ...
              >
                ...
                {/* Make sure Award is imported from lucide-react at the top */}
                Skill Assessment
              </button>
            </div>
          )}
          ...
          {activeTab === "kra-kpi" && currentUser && jd && (
            <div className="p-5 sm:p-8">
              {/* 
       FIX: Always render the KRAKPIPanel. 
       If no KRA/KPI exists yet, the panel itself will show the 
     ...
     */}
              <KRAKPIPanel
                ...
                isManager={canActOnThisJD}
                isDirectManager={isManagerRole && isDirectManager}
                isHRUser={isHRUser}
                jdData={jd}
                hideManagerActions={true}
              />
            </div>
          )}
          {/* Skill Assessment tab content */}
          {activeTab === "skill-assessment" && currentUser && jd && (
            <div className="p-5 sm:p-8">
              <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-md p-6 text-center">
                ...
                <p className="text-sm">
                  Your KRA & KPI is approved. Your skill assessment component
                  will render here.
                </p>

                {/* TODO: Replace the above placeholder with your actual Skill Assessment component when ready */}
                {/* Example: <SkillAssessmentPanel jdId={jdId} employeeId={jd.employee_id} /> */}
              </div>
            </div>
          )}
        ...
        <FeedbackModal
          ...
          jdSessionId={jdId}
          defaultCategory={
            jd?.status === "approved" ? "KRA/KPI Process" : "JD Process"
          }
        />
